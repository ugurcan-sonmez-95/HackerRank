...
bool validation(const std::vector<char> &chars_to_validate) {
...
int maxAlternateStringSize(const std::string &s) {
    int max_size{};     
    const std::set<char> set_of_chars(s.begin(), s.end()); // Gets the unique elements as unsorted from s
    const std::vector<char> vec_of_chars(set_of_chars.begin(), set_of_chars.end());
...
int main() {
    ...
    const int result = maxAlternateStringSize(s);
    std::cout << result;
